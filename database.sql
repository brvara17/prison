...
`eye_color`.`status_english` AS `marital_status_english`,
`eye_color`.`status_dari` AS `marital_status_dari`,
`eye_color`.`status_pashto` AS `marital_status_pashto`,
`prisoner`.`present_province_id` AS `present_province_id`,
`present_province`.`name_english` AS `present_province_english`,
